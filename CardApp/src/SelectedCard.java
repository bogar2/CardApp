...
public class SelectedCard extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					SelectedCard frame = new SelectedCard();
	...
	public SelectedCard() {
		...
		JButton hatlapkiv = new JButton("H\u00E1tlap kiv\u00E1laszt\u00E1sa");
		hatlapkiv.setBackground(Color.BLACK);
		hatlapkiv.setFont(new Font("Arial Rounded MT Bold", Font.BOLD, 13));
		hatlapkiv.setForeground(Color.BLACK);
		hatlapkiv.setBorderPainted(true);
		hatlapkiv.setContentAreaFilled(false);
		hatlapkiv.setFocusPainted(false);
		hatlapkiv.setOpaque(false);
		
		hatlapkiv.addActionListener(new ActionListener() {
		...
		hatlapkiv.setBounds(569, 592, 177, 23);
		contentPane.add(hatlapkiv);
		...
		merge.setBounds(793, 592, 150, 23);
		...
		Document document = new Document(PageSize.A4);		// DOK PÉLDÁNY LÉTREHOZÁSA MÉRET
		document.setMargins(0, 0, 0, 0);		// MARGÓ
		try {
			PdfWriter.getInstance(document, new FileOutputStream("C:\\Users\\Bimba\\Desktop\\picsfortheprog\\kartyak.pdf")); // 	FÁJL STREAM LÉTREHOZÁSA
		...
		btnNewButton.addActionListener(new ActionListener() {
			...
			BufferedImage buf = createImage(panel);
					...
					try {
						...
						// TODO Auto-generated catch block
						e1.printStackTrace();
					}	// ÁTALAKÍTÁS IMAGE ÁLTAL ÉRTELMEZHETÕ FORMÁTUMRA
					try {
						Image iTextImage = Image.getInstance(baos.toByteArray());		// MENTÉS IMAGE TÍPUSRA 
						iTextImage.setAlignment(Element.ALIGN_CENTER);		// KÖZÉPRE IGAZÍTÁS 
						try {
							document.add(iTextImage);		// HOZZÁADÁS A DOKSIHOZ 
						} catch (DocumentException e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
						}
		...
		hatter.setIcon(new ImageIcon(SelectedCard.class.getResource("/image/hatterveg.jpg")));
